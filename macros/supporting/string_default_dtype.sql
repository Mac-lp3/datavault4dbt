...
{%- if global_var is mapping -%}
    ...
    {%- else -%}
        {%- if execute -%}
            {%- do exceptions.warn("Warning: You have set the global variable 'datavault4dbt." ~ type ~ "_default_dtype' to a dictionary, but have not included the adapter you use (duckdb) as a key. Applying the default value.") -%}
        {% endif %}
        {%- set string_default_dtype = "VARCHAR" -%}
    {% endif %}
{%- elif global_var is not mapping and datavault4dbt.is_something(global_var) -%}
